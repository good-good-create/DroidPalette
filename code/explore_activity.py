...
def scan_and_return():
    ...
    os.system(f'{adb} shell input tap 540 519')
    time.sleep(5)
...
def collect_results(activity, appname, accessbility_folder, results_outputs):
    ...
    if os.path.exists(os.path.join(issue_path, f"{activity}.zip")):
        ...
        cur_file = os.path.join(issue_path,activity)
...
def check_current_screen_new(activity, appname, results_outputs):
    ...
    for word in keywords:
        cmd = f'findstr "{word}" {layout_path}'
        result = subprocess.run(cmd, shell=True, text=True, encoding="utf-8", stdout=subprocess.PIPE, stderr=subprocess.PIPE).stdout
...
def parseManifest(new_apkpath, apk_name, results_folder, decompilePath, results_outputs, act_name):
...
def get_package_name(apk_path):
    ...
    result = subprocess.check_output(cmd, shell=True, text=True, stderr=subprocess.STDOUT, encoding='utf-8')
...
def get_pkgname(apk_path):
    ...
    aapt_output = subprocess.check_output(cmd, shell=True, text=True, stderr=subprocess.STDOUT, encoding='utf-8')
...
def exploreActivity(new_apkpath, apk_name, results_folder, emulator, tmp_file, storydroid, act_name):
    ...
    if result == 'Failure':
        # Copy original app to install error folder (Windows)
        copy_org_apk = f"move {new_apkpath} {installErrorAppPath}"
        subprocess.getoutput(copy_org_apk)
        return

    parseManifest(new_apkpath, apk_name, results_folder, decompilePath, results_outputs,act_name)
